...
- **알려진 제약사항**: 현재 브라우저 로컬 스토리지(LocalStorage)에 데이터를 임시로 백업하는 저장 기능은 구현되어 있지 않으며, 최종 저장을 클릭하지 않은 상태에서 브라우저 탭을 닫거나 새로고침하면 작성 중이던 로컬 변경 사항은 초기화됩니다. (대신 이탈 전 `beforeunload` 경고창을 띄워 데이터가 날아가지 않도록 안전하게 사전 예방하고 있습니다.)
...
- **실제 사용한 MCP 도구 및 검증 (테스트)**:
  ...
  - **실제 검증 내역**:
    ...
    - **30분 단위 인접 경계값 및 시간 충돌 로직 브라우저 검증**:
      - **인접 시간대 추가 성공 테스트**: 월요일 기존 일정 `09:00 ~ 11:00` 바로 옆인 `11:00 ~ 12:00`에 신규 일정을 등록 시 충돌 없이 정상적으로 스케줄이 추가 및 그리드에 안착함을 확인.
        ![인접 시간대 추가 성공테스트 사지](/public/images/adjacent_block_added.png).
      - **다중 충돌 및 경고 배너 표출 테스트**: 월요일 `10:00 ~ 11:30` (기존 두 일정과 부분 겹침) 범위로 추가 시도 시, 저장 차단 및 상단에 다중 충돌 목록(`• [월요일 09:00~11:00 파이썬 기초]`, `• [월요일 11:00~12:00 리액트]`)이 빨간색 경고 배너로 출력됨을 확인.
        ![다중 충돌 및 경고 배너 표출 테스트 사진](/public/images/multiple_overlaps_conflict.png)
